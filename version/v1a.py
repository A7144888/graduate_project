...
FORECAST_AFTER_DATE = "2025-06-20" 
...
# 依「參考日之後三個交易日」截斷建模資料；保留完整收盤以利 Forecast 對照
...
print(
    "[Data] 參考日(之後為預測窗起點): "
    f"{pd.Timestamp(FORECAST_AFTER_DATE).strftime('%Y-%m-%d')}  |  "
    f"錨點(最後訓練日): {_anchor_ts.strftime('%Y-%m-%d')}  |  "
    "預測日: "
    + ", ".join(d.strftime("%Y-%m-%d") for d in _forecast_dates)
)
...
# 方向命中率：逐元素比較 pred 與 actual 是否同號（漲 / 跌）
